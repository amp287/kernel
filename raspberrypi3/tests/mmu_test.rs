...
fn simple_write() {
    ...
    for (index, address) in (0x80000_u64..0x8A000_u64).step_by(0x1000).enumerate() {
    ...
    phys_addr = PhysicalAddress::new(0x1000);

    // check to see if this physical address is ok to be used (as in its not a register and gpu doesnt change here).
    block_entry = BlockDescriptor::new(PhysicalAddress::new(0x1000), GranuleSize::_4KB, block_attrib);
